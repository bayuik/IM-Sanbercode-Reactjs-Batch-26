  ...
  const handleSubmit = () => {
    ...
    if (title.replace(/\s/g, "") !== "") {
      if (statusForm === "edit") {
        axios
          .put(
            `https://www.backendexample.sanbersy.com/api/movies/${id}`,
            {
              title: movies.title,
          ...
          .catch((err) => {
            message.error(
              "Mohon Maaf, terdapat kesalahan dalam penginputan data"
            );
          });
      }
        ...
        <div style={{ border: "1px solid #aaa", padding: "20px" }}>
          ...
          >
            <Form.Item label="Tittle" style={{ fontWeight: "bold" }}>
              <Input
                placeholder="Avatar, Naruto, ..."
                type="text"
                name="title"
                value={movies !== null && movies.title}
            ...
            <Form.Item label="Genre" style={{ fontWeight: "bold" }}>
              <Input
                ...
                type="text"
                name="genre"
                value={movies !== null && movies.genre}
                onChange={handleChange}
                required
              />
            </Form.Item>
            ...
            <Form.Item label="Duration" style={{ fontWeight: "bold" }}>
              <Input
                type="number"
                name="duration"
                value={movies !== null && movies.duration}
                onChange={handleChange}
                required
              />
            </Form.Item>
            <Form.Item label="Rating" style={{ fontWeight: "bold" }}>
              <Input
                ...
                max={10}
                min={0}
                value={movies !== null && movies.rating}
                onChange={handleChange}
                required
              />
            </Form.Item>
            <Form.Item label="Description" style={{ fontWeight: "bold" }}>
              <Input.TextArea
                required
                placeholder="Film ini adalah..."
                cols="40"
                rows="3"
                name="description"
                value={movies !== null && movies.description}
                onChange={handleChange}
              />
            </Form.Item>
            <Form.Item label="Review" style={{ fontWeight: "bold" }}>
              <Input.TextArea
                required
                placeholder="Menururt saya, Film ini merupakan..."
                cols="40"
                rows="3"
                name="review"
                value={movies !== null && movies.review}
                onChange={handleChange}
              />
            </Form.Item>
            <Form.Item label="Image URL" style={{ fontWeight: "bold" }}>
              <Input.TextArea
                ...
                rows="3"
                name="image_url"
                value={movies !== null && movies.image_url}
                onChange={handleChange}
              />
            </Form.Item>
